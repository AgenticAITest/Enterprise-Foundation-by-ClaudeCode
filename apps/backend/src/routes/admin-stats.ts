    ...
    try {
      ...
      try {
        ...
        const tenantCheck = await client.query(
          'SELECT id, company_name, status FROM tenants WHERE id = $1',
          [tenantId]
        );
        ...
        `, [
          ...
          'tenant',
          tenantId,
          JSON.stringify({ reason, suspended_by: adminUser?.email }),
          'critical'
        ]);
        
        res.json({
          status: 'success',
          message: `Tenant "${tenant.company_name}" has been suspended`,
          data: {
            tenant_id: tenantId,
